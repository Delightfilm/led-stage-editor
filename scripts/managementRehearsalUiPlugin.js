  ...
  return {
    ...
    transform(code, id) {
      ...
      const instantArm = [
        ...
        "    // Use the real media clock, not React's frame-delayed currentTime.",
        ...
        "    setCurrentTime(clamp(liveTime, 0, duration))",
        "    bStartSentRef.current = true",
        "    spaceResumeRef.current = false",
        "    bLivePrimedRef.current = true",
        "    setStageMode('B_LIVE')",
        "",
        "    // When B LIVE starts from PAUSE, the browser needs a small, variable amount of",
        "    // time before audio/video actually begins moving. Sending RF before that point",
        "    // makes EL cues appear early. Wait for real playback, then sample the clock again.",
        "    if (!wasPlaying) {",
        ...
        "      return",
        "    }",
        "    showToast(`${rehearsalMode ? '연습실' : 'B'} LIVE GO · ${fmtTime(goOffsetMs / 1000)}${userLeadMs > 0 ? ` · START LEAD ${userLeadMs}ms` : ''}`)",
        "  }",
      ].join('\n')
      ...
      if (!out.includes('const forceStopRehearsal = async')) {
        ...
        out = out.replace(selectAnchor, [
          ...
          "    if (mediaEl && Number.isFinite(holdTime)) { try { mediaEl.currentTime = holdTime } catch {} }",
