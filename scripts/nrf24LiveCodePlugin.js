    ...
    transform(code, id) {
      ...
      if (!out.includes("const receiverExportTargets = useMemo")) {
        ...
        const helpers = [
          ...
          '    if (!selectedReceiverTarget) return { events: 0, bytesPerEvent: 2, bytes: 0 };',
          ...
          '    return { events: frames.length, bytesPerEvent, bytes: frames.length * bytesPerEvent };',
      ...
      if (!out.includes(panelMarker)) {
        ...
        const panel = [
          ...
          '                ? "MASTER · 타임라인 데이터 없음"',
          '                : "예상 타임라인 데이터: " + formatNrfBytes(selectedReceiverStorageEstimate.bytes) + " · " + selectedReceiverStorageEstimate.events + " events × " + selectedReceiverStorageEstimate.bytesPerEvent + " B"}',
          '            </p>',
          ...
          '                {liveCodeTarget === "master" ? masterReceiverCount + "대 · " + Math.round(showDurationMs / 1000) + "s" : formatNrfBytes(selectedReceiverStorageEstimate.bytes)}',
          '              </span>',
          '            </div>',
