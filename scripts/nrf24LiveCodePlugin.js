  ...
  return {
    ...
    transform(code, id) {
      ...
      if (!out.includes("const receiverExportTargets = useMemo")) {
        ...
        const helpers = [
          '  // ───────────── nRF24 code generator / show fingerprint ─────────────',
          ...
          '    feed(showDurationMs);',
          '    receiverExportTargets.forEach((target) => {',
          '      feed(target.receiverId);',
          ...
          '    });',
          '    return hash >>> 0;',
          '  }, [receiverExportTargets, masterReceiverCount, showDurationMs]);',
          '',
          ...
          '      return buildNrf24MasterSketch({ receiverCount: masterReceiverCount, showDurationMs, showHash });',
          '    }',
          '    return buildNrf24ReceiverSketch({ ...selectedReceiverTarget, showHash });',
          '  }, [liveCodeTarget, selectedReceiverTarget, masterReceiverCount, showDurationMs, showHash]);',
          '',
      ...
      if (!out.includes(panelMarker)) {
        ...
        const panel = [
          ...
          '                : "타임라인 " + formatNrfBytes(selectedReceiverStorageEstimate.bytes) + " · 예상 전체 Flash " + formatNrfBytes(selectedReceiverStorageEstimate.flashMin) + "~" + formatNrfBytes(selectedReceiverStorageEstimate.flashMax) + " / 31.5 KB · " + selectedReceiverStorageEstimate.status}',
          '            </p>',
          ...
          '                {liveCodeTarget === "master" ? showHashText : selectedReceiverStorageEstimate.pctMax + "% upper est."}',
          '              </span>',
          '            </div>',
