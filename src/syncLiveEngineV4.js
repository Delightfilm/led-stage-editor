...
// V4 field tracking: "no evidence" is not the same as "wrong evidence".
const COAST_MAX_FRAMES = 80              // 8.0 s at 100 ms/frame
const COAST_SOFT_FRAMES = 24             // confidence begins a slow decay after 2.4 s
const CONTRADICTION_MIN_JUMP = 30         // >= 3.0 s away from predicted clock
const CONTRADICTION_STREAK = 14           // >= 1.4 s sustained strong contradiction
const CONTRADICTION_DECAY = 2
const TRACK_CORRECTION_LIMIT = 2          // max +/-200 ms correction per feature frame
...
export class SyncMatcher {
  ...
  push(feature) {
    ...
    if (this.state === 'LOCKED' && predicted != null) {
      // The clock is authoritative while locked. Evidence may correct it slightly,
      // but missing evidence only starts COASTING; it does not freeze the playhead.
      spectral = bestLocalSpectral(this.live, ref, predicted, LOCK_LOCAL_RADIUS)

      const landmarkNear = landmarkCurrentIndex != null && Math.abs(landmarkCurrentIndex - predicted) <= LOCK_LOCAL_RADIUS
      const landmarkConsistent = landmarkNear && cluster.hits >= 4 && cluster.consensus >= .46
      const spectralGood = spectral.score >= .34 && spectral.index >= 0 && Math.abs(spectral.index - predicted) <= 4
      const localEvidence = landmarkConsistent || spectralGood

      let evidenceIndex = predicted
      if (landmarkConsistent && spectralGood) {
        const landmarkDistance = Math.abs(landmarkCurrentIndex - predicted)
        const spectralDistance = Math.abs(spectral.index - predicted)
        evidenceIndex = landmarkDistance <= spectralDistance ? landmarkCurrentIndex : spectral.index
      } else if (landmarkConsistent) evidenceIndex = landmarkCurrentIndex
      else if (spectralGood) evidenceIndex = spectral.index

      const correction = localEvidence ? clamp(Math.round(evidenceIndex - predicted), -TRACK_CORRECTION_LIMIT, TRACK_CORRECTION_LIMIT) : 0
      chosenIndex = clamp(Math.round(predicted + correction), 0, ref.length - 1)
      continuityOk = Math.abs(chosenIndex - predicted) <= TRACK_CORRECTION_LIMIT

      // Strong evidence at a very different song position is treated separately from
      // ordinary noise. It must be both spectrally plausible and persistent.
      let contradictionStrong = false
      if (landmarkCurrentIndex != null && Math.abs(landmarkCurrentIndex - predicted) >= CONTRADICTION_MIN_JUMP &&
          cluster.hits >= 12 && cluster.consensus >= .52 && cluster.bestWeight >= 2.4) {
        const remote = bestLocalSpectral(this.live, ref, landmarkCurrentIndex, 4)
        contradictionStrong = remote.score >= .36
      }

      if (contradictionStrong) this.contradictionStreak += 1
      else this.contradictionStreak = Math.max(0, this.contradictionStreak - CONTRADICTION_DECAY)
      ...
      if (localEvidence) {
        ...
        this.weakFrames += 1
        this.trackingMode = 'COASTING'
      }

      this.lastIndex = chosenIndex
      this.lockFrames += 1

      // "No evidence" may coast for up to 8 seconds. A sustained, high-quality
      // contradiction can release the lock sooner so a real seek/restart can reacquire.
      if (this.contradictionStreak >= CONTRADICTION_STREAK || this.weakFrames >= COAST_MAX_FRAMES) {
        this.state = 'LOST'
        this.trackingMode = this.contradictionStreak >= CONTRADICTION_STREAK ? 'REACQUIRE' : 'LOST'
        this.candidateStreak = 0
        this.candidateIndex = null
        this.lockFrames = 0
      }
      ...
      if (acceptable) {
        ...
        if (this.candidateStreak === 0) {
          ...
          if (this.state !== 'LOST') {
            this.state = 'SEARCHING'
            this.trackingMode = 'SEARCHING'
          } else if (this.trackingMode !== 'REACQUIRE') this.trackingMode = 'LOST'
        }
      }
    }
    ...
    // Field confidence: landmark history + clock continuity dominate; whole-spectrum
    // similarity is deliberately a minor term because speech/PA coloration damages it.
    let confidenceValue = landmarkScore * .54 + continuityPart * .26 + spectralPart * .12
    if (uniquenessMargin != null) confidenceValue += clamp(uniquenessMargin / .12, 0, 1) * .08
    else confidenceValue += .04

    if (this.state === 'LOCKED') {
      const stableRamp = clamp(this.lockFrames / 10, 0, 1)
      if (this.trackingMode === 'TRACKED') {
        const trackedFloor = .76 + .12 * stableRamp + .05 * landmarkScore + .03 * spectralPart
        confidenceValue = Math.max(confidenceValue, trackedFloor)
      } else {
        const coastProgress = clamp(this.weakFrames / COAST_MAX_FRAMES, 0, 1)
        const softProgress = clamp((this.weakFrames - COAST_SOFT_FRAMES) / Math.max(1, COAST_MAX_FRAMES - COAST_SOFT_FRAMES), 0, 1)
        const coastFloor = (0.74 - 0.26 * softProgress) * (1 - 0.08 * coastProgress)
        confidenceValue = Math.max(confidenceValue, coastFloor)
      }
    }

    if (this.state === 'SEARCHING') confidenceValue *= .72
    if (this.state === 'LOST') confidenceValue *= .62
    ...
    return {
      ...
      coastingSec: this.state === 'LOCKED' && this.trackingMode === 'COASTING' ? this.weakFrames * this.reference.frameSec : 0,
      landmarkOffsetFrames: cluster.offset,
      weakFrames: this.weakFrames,
      contradictionStreak: this.contradictionStreak,
...
export const SYNC_LIVE_CONSTANTS = {
  ...
  matcherVersion: 'V4 FIELD TRACK',
}

export const __SYNC_LIVE_TESTING__ = {
  framePeaks,
  buildLandmarkIndex,
  hashKey,
  clusterVotes,
